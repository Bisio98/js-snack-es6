...
students.forEach((element,index,array) => {
    studentsUpper.push(element.name.toUpperCase());
});
...
const gradesUpper70 = students.filter((element,index,array) => {
    return element.grades > 70;
})

console.log(gradesUpper70);

// 3
const gradesUpperAndId = students.filter((element,index,array) =>{
    return element.grades > 70 && element.id > 120;
})

console.log(gradesUpperAndId);